...
Build output is the single file `dist/ClippyPet-v0.2.2.exe` (onefile) — double-click to run. **No Python needed**; the single file runs directly.
...
```bash
python -m PyInstaller --noconfirm --clean ClippyPet-v0.2.2.spec
```
- Output: `dist/ClippyPet-v0.2.2.exe` (onefile, bundles all skin assets)
...
├── ClippyPet-v0.2.2.spec   # PyInstaller build config
