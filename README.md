...
### 方式一：直接运行 exe（推荐）
打包产物为 `dist/ClippyPet-v0.2.2.exe`（onefile 单文件），双击即可，**无需安装 Python**；单文件可直接运行。
...
```
- 产物：`dist/ClippyPet-v0.2.2.exe`（onefile，内嵌全部皮肤素材）
...
├── ClippyPet-v0.2.2.spec   # PyInstaller 打包配置
